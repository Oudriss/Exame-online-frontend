...
public class ExamenFilter implements Filter {
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException {
		...
		if( (session.getAttribute("Utilisateur") != null) && (req.getParameter("action").equals("Valider les réponses")) 
				&& ((ExamenTimer.END_TIME - ExamenTimer.START_TIME) >= 7200000)  ){
			System.out.println((ExamenTimer.END_TIME - ExamenTimer.START_TIME) + "Timing ==");
			req.setAttribute("Erreur", "Vous avez depassé la durée d'examen, vos réponses !!");
